...
export const createPlayerAbility = (
  ...
  const canAct = (e: Entity) => e.actionsTaken < e.maxActions;
  ...
  return createAbility<PlayerAbility>(({ can, cannot }) => {
    ...
    can('use_skill', 'entity', (subject: Entity) => {
      return isOwnedAndActive(subject) && canAct(subject);
    });
    ...
    if (canAct(general)) {
      ...
      can('summon_at', 'position', (subject: Point) => {
        ...
        return surroundingCells.some(
          cell => getEntityAt(state, cell)?.owner === playerId
        );
      });
    }
  });
