...
export const zid = <TableName extends TableNames>(tableName: TableName) =>
  z.custom<Id<TableName>>(val => typeof val === 'string').pipe(z.coerce.string());
 ...
 */
export const withZod = <Ctx, Args extends Record<string, z.ZodTypeAny>, Returns>({
  args,
  handler
}: {
  args: Args;
...
>(argObject: {
  args: Args;
  handler: (ctx: QueryCtx, arg: z.output<z.ZodObject<Args>>) => Promise<Returns>;
...
>(argObject: {
  args: Args;
  handler: (ctx: MutationCtx, arg: z.output<z.ZodObject<Args>>) => Promise<Returns>;
